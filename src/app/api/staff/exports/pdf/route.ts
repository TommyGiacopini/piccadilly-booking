...
import { pdfExportRequestSchema } from "@/modules/exports/domain/export-domain";
...
export async function POST(request: Request): Promise<Response> {
  ...
  const parsed = pdfExportRequestSchema.safeParse(rawPayload);
  if (!parsed.success) return invalidExportRequest();

  try {
    return downloadResponse(
      await exportService.generatePdfExport({
        actor: {
